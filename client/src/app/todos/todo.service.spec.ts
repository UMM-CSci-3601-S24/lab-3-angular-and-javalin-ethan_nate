  ...
  describe('getTodos()', () => {
        ...
        describe('Calling getTodos() with parameters correctly forms the HTTP request', () => {
          ...
          it('correctly calls api/todos with filter parameter \'admin\'', () => {
            ...
            expect(req.request.params.get('status') === 'true').toEqual(true);
            req.flush(testTodos);

          });

          it('correctly calls api/todo with category filter parameters', () => {

            todoService.getTodos({category: 'IBM' }).subscribe(
              todos => expect(todos).toBe(testTodos)
            );
            ...
            expect(req.request.params.get('category')).toEqual('IBM');
        ...
        it('correctly calls api/todo with body filter parameters', () => {

          todoService.getTodos({body: 'pat@something.com' }).subscribe(
            todos => expect(todos).toBe(testTodos)
          );
          ...
          // Check that the role, company, and age parameters are correct
          expect(req.request.params.get('body')).toEqual('pat@something.com');
...
describe('filterTodos()', () => {
  ...
  it('filters by category', () => {
    ...
    // There should be just one todo that has UMM as their category.
    expect(filteredTodos.length).toBe(1);
    // Every returned todo's category should contain 'UMM'.
    filteredTodos.forEach(todo => {
  ...
  it('filters by owner and category', () => {
    ...
    // contains an 'i' and whose category contains
    // an 'M'. There are two whose owner contains
    // an 'i' and two whose category contains an
